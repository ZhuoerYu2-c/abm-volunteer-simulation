...
DATA_PATHS = [
    os.path.join(SCRIPT_DIR, "..", "data", "empirical", "大学生志愿者挫折反应及对策调查数据.csv"),
...
SPSS_PATHS = [
    os.path.join(SCRIPT_DIR, "..", "data", "empirical", '"大学生志愿者挫折反应及对策调查"数据.sav'),
    os.path.join(SCRIPT_DIR, "..", "data", "empirical", "大学生志愿者挫折反应及对策调查数据.sav"),
]
SAV_PATH = None
for p in SPSS_PATHS:
    if os.path.exists(p):
...
out_df.to_csv(os.path.join(SCRIPT_DIR, "..", "data", "empirical", "09_three_regression_comparison_final.csv"), index=False)
print(f"\n已保存: data/empirical/09_three_regression_comparison_final.csv")
...
# 保存
ct.to_csv(os.path.join(SCRIPT_DIR, "..", "data", "empirical", "10_frustration_attrition_crosstab.csv"))

# ============================================================
# 9. 保存描述性统计
# ============================================================
desc_cols = ['挫折感', '志愿服务现状满意度', '服务总时长', '参与项目总数',
             'high_grade', 'is_manager', '服务中断']
desc_df = df_main[desc_cols].describe().T
desc_df.columns = ['计数', '均值', '标准差', '最小', '25%', '中位数', '75%', '最大']
desc_df['备注'] = ['1-5级(越高挫折感越强)', '1-5级(已正向编码)',
                   '累计时间', '项目总数', '年级>=3为1', '角色1/2=管理层', '0=未流失,1=流失']
desc_df.to_csv(os.path.join(SCRIPT_DIR, "..", "data", "empirical", "11_descriptive_statistics.csv"))
print("\n描述性统计已保存")
